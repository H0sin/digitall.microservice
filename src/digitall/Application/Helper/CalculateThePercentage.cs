...
public class Percent(IAgentService agentService, IMarzbanService? marzbanService = null) : IDisposable
{
    ...
    public async Task<GetMarzbanVpnDto?> CalcuteVpnPrice(long vpnId, long userId, int numberOfAgents = 2)
    {
     ...
     public async Task<GetMarzbanVpnDto?> CalcuteVpnPrice(long vpnId, long userId,int totalGb,int totalDay,int numberOfAgents = 2)
    {
        AgentDto? admin = await agentService.GetAgentByAdminIdAsync(userId);
